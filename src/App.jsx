    ...
    <div className="app">
      ...
      <div className="layout">
        ...
        <div className="main">
          ...
          {/* CİHAZ DETAY */}
          {ekran === "cihazDetay" && seciliCihaz && (() => {
            // Cihaz türüne göre tüm bakım işlemleri
            const tur = seciliCihaz.ad;
            let tumIslemler = [];
            if (tur === "Hormon") {
              tumIslemler = ["Pinch Tube Değişimi", "Dekon Yapıldı", "Bidon Temizliği", "Hat Bakımı", "Bakım Kiti", "Hücre Değişimi"];
            } else if (tur === "ISE") {
              tumIslemler = ["Bakım Kiti", "Dekon Yapıldı"];
            } else if (tur === "Biyokimya") {
              tumIslemler = ["Bakım Kiti", "Dekon Yapıldı", "Vacuum Tank", "Bidon Temizliği"];
            }
            // Her işlem için en son yapılma tarihi
            function sonIslemTarih(islemAdi) {
              const l = bakimlar.filter(b => b.cihazId === seciliCihazId && b.notlar && b.notlar.includes(islemAdi));
              if (l.length === 0) return null;
              return l.reduce((en, b) => (!en || new Date(b.tarih) > new Date(en)) ? b.tarih : en, null);
            }
            function islemSkor(tarih) {
              const g = gunFarki(tarih);
              if (g === null) return 2; // hiç yapılmamış = kritik
              if (g <= 30) return 0;
              if (g <= 45) return 1;
              return 2;
            }
                  ...
                  <div className="d-meta">
                    ...
                    {seciliCihaz.seri && (
                      <span style={{display:"inline-flex",flexDirection:"column",lineHeight:1.3}}>
                        <span style={{fontSize:9,color:"#999",letterSpacing:"1px",textTransform:"uppercase"}}>{seciliCihaz.ad} S/N</span>
                        <span style={{color:"#1a1a1a",fontWeight:700,fontSize:14,letterSpacing:"0.5px"}}>{seciliCihaz.seri}</span>
                      </span>
                    )}
                  </div>
                  {/* Bakım durum özeti */}
                  {tumIslemler.length > 0 && (
                    <div style={{marginTop:12,display:"flex",flexWrap:"wrap",gap:6}}>
                      {tumIslemler.map(islem => {
                        const tarih = sonIslemTarih(islem);
                        const skor = islemSkor(tarih);
                        const renk = skor === 0 ? {bg:"#dcfce7",border:"#bbf7d0",text:"#16a34a",icon:"✓"}
                                   : skor === 1 ? {bg:"#fef9c3",border:"#fef08a",text:"#a16207",icon:"⚠"}
                                   : {bg:"#fee2e2",border:"#fecaca",text:"#dc2626",icon:"✗"};
                        const gun = gunFarki(tarih);
                        return (
                          <div key={islem} style={{
                            background:renk.bg,border:`1px solid ${renk.border}`,
                            borderRadius:2,padding:"4px 9px",fontSize:10,
                            color:renk.text,fontWeight:600,letterSpacing:"0.3px",
                            display:"flex",alignItems:"center",gap:4
                          }}>
                            <span>{renk.icon}</span>
                            <span>{islem}</span>
                            {tarih && <span style={{fontWeight:400,opacity:0.8}}>({gun}g)</span>}
                            {!tarih && <span style={{fontWeight:400,opacity:0.8}}>(Yok)</span>}
                          </div>
                        );
                      })}
                    </div>
