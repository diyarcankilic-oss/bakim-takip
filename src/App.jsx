...
const LINES = Array.from({length:8}, (*,i) => `${i+1} LINE`);
...
const yeni = yedekTablolar.map(t => t.id === tabloId
? { …t,
sutunlar: t.sutunlar.filter((*, i) => i !== kolIdx),
satirlar: t.satirlar.map(s => ({ …s, hücreler: s.hücreler.filter((*, i) => i !== kolIdx) }))
}
: t
);
setYedekTablolar(yeni);
                          ...
                          return (
                            <tr key={c.id} style={{cursor:"pointer"}} onClick={() => { goKurum(c.kurumId); setTimeout(()=>goCihaz(c.id),50); }}>
                              <td style={{fontWeight:600}}>
                                <div>{c.ad}</div>
                                <div style={{fontSize:10,color:"#999",fontWeight:400,marginTop:3}}>{c.ad} S/N</div>
                                {c.seri && <div style={{fontSize:11,fontWeight:600,color:"#1a1a1a"}}>{c.seri}</div>}
                              </td>
          ...
          {cihazBakimlari.length === 0 ? (
            ...
            <>
              ...
              {(() => {
                ...
                return (
                  <div style={{marginBottom:20,padding:12,background:"#faf8f4",border:"1px solid #e5ddd0",borderRadius:4}}>
                    <div style={{fontSize:10,letterSpacing:1,textTransform:"uppercase",color:"#999",marginBottom:10,fontWeight:600}}>İşlem Durumu</div>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                      {islemler.map(i => {
                        const renkler = { ok: "#16a34a", warn: "#eab308", kritik: "#dc2626" };
                        return (
                          <div key={i.islem} style={{fontSize:11,padding:8,background:"white",border:`2px solid ${renkler[i.durum]}`,borderRadius:3,color:renkler[i.durum],fontWeight:600}}>
                            <div>{i.islem}</div>
                            {i.durum === "ok" ? <span>✓</span> : i.durum === "warn" ? <span>⚠ {i.gunSayisi}g</span> : <span>✗ {i.gunSayisi === null ? "Yok" : i.gunSayisi + "g"}</span>}
                          </div>
                        );
                      })}
                    </div>
